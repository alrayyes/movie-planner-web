...
This app has zero TMDb integration today — fields like `trailerUrl` only ever arrive already-written by the CLI. The CLI enriches every matched viewing with TMDb data (trailer, full cast, collection, certification, keywords, budget, popularity) once it has an `imdb_id` from OMDb; this app has no equivalent, so a viewing logged or refreshed entirely through the web app never gets that data at all. Tracked as alrayyes/movie-planner-web#360 and #400.

## What Changes

- New `src/lib/tmdb/client.ts`: resolves TMDb's numeric id via `find/{imdb_id}` (never a title/year search — TMDb only ever runs when an `imdb_id` is already known), then fetches `movie/{tmdb_id}?append_to_response=credits,videos,release_dates,keywords` in one round trip.
...
- `movie-log`: "Best-effort OMDb enrichment" requirement gains the same TMDb step once OMDb resolves (or the disambiguation picker attaches) an `imdb_id`.
